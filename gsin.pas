...
var
  dx, x0, xk,  // переменные, относящиеся к поставленной задаче
...
begin
  ...
  (x0, xk) := (-2 * pi, 2 * pi); // начальное и конечное значения по Х
  dx := 0.001; // шаг по Х
  // задание переменных для работы с графикой
  Hu := Window.Width / 2; //см. рис. 2(в)
  Hv := Window.Height / 2; // (-1) -- чтобы ось не ушла под экран
  ...
  Sy := Sx;
  Window.Caption := 'Синус и косинус'; // Заголовок окна
...
begin
  ...
  begin
    var y := sin(x); // Вычисляем значение функции sin в точке х
    OutPixel(x, y, colors.Red); // Рисуем точку красным
    y := cos(x); // Вычисляем значение функции cos в точке х
    OutPixel(x, y, colors.Blue); // Рисуем точку красным
